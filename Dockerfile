...
RUN chmod +x /app/init-libs.sh /app/run-audiveris.sh /app/run-audiveris-system.sh /app/preload-javacpp.sh /app/download-javacpp-libs.sh /app/download-manual-javacpp.sh /app/download-and-bundle-jni.sh /app/use-existing-jni.sh /app/extract-jni-simple.sh
...
# Comando por defecto extrayendo JNI de JARs existentes
CMD ["sh", "-c", "./init-libs.sh && ./extract-jni-simple.sh && npm start"]
